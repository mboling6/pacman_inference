...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        possibilities = util.Counter()
        for oldPos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for newPos, probability in newPosDist.items():
                possibilities[newPos] += probability * self.beliefs[oldPos]
        self.beliefs = possibilities
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            tmp[particle] += self.getObservationProb(observation, gameState.getPacmanPosition(), particle, self.getJailPosition())
        tmp.normalize()
        ...
        if tmp.total() == 0:
            ...
            for i in range(self.numParticles):
                newParticleList.append(tmp.sample())
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        #same implementation as in ParticleFilter
        while len(self.particles) < self.numParticles:
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        #similar implementation to ParticleFilter as explained above
        tmp = DiscreteDistribution()
        for particle in self.particles:
            baseProbability = 1.0
            for ghost in range(self.numGhosts):
                baseProbability *= self.getObservationProb(observation[ghost], gameState.getPacmanPosition(), particle[ghost], self.getJailPosition(ghost))
            tmp[particle] += baseProbability
        tmp.normalize()

        #identical implementation to ParticleFilter at the end
        if tmp.total() == 0:
            self.initializeUniformly(gameState)
        else:
            newParticleList = []
