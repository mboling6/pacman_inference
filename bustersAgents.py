...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for pos in livingGhostPositionDistributions:
            mostLikelyPos.append(pos.argMax())
        
        goalPos = None
        goalProbability = 0
        for i, pos2 in enumerate(mostLikelyPos):
            if livingGhostPositionDistributions[i][pos2] >= goalProbability:
                    goalPos = pos2
                    goalProbability = livingGhostPositionDistributions[i][pos2]

        #iterate through actions
        actionsTuple = []
        for action in legal:
            nextPos = Actions.getSuccessor(pacmanPosition, action) #get successor position
            actionsTuple.append((self.distancer.getDistance(nextPos, goalPos), action)) #add distance action pair to tuple
        
        #compute the best action
        minPair = min(actionsTuple, key = lambda t: t[0])
        bestAction = minPair[1]
